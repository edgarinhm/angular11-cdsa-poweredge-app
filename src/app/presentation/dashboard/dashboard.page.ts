...
export class DashboardPage implements OnInit {
  ...
  getUsers(){
    this.users = [];
    this.userUsecase.get().subscribe(
      (value: UserModel) => { 
  ...
  public onAddUser(addForm: NgForm): void {    
    ...
    this.userUsecase.add(addForm.value).subscribe(
      (response: UserModel) => {
  ...
  public onUpdateUser(user: UserModel): void {
    this.userUsecase.update(user).subscribe(
      (response: UserModel) => {
        console.log(response);
        this.getUsers();
      },
  ...
  public onDeleteUser(userId: number): void {
    this.userUsecase.delete(userId).subscribe(
      (response: void) => {
        console.log(response);
        this.getUsers();
      },
  ...
  public searchUsers(key: string): void {
    ...
    //if (results.length === 0 || !key) {
      this.userUsecase.findByName(key).subscribe(
        (value: UserModel) => { 
          console.log(value)
          results.push(value);
       });
